...
//  *
//  * HOW TO USE (typical):
//  * - Call the hook inside your component and attach the returned `bind` to the draggable lane (or active card wrapper).
//  * - While dragging, apply `style={{ transform: \`translateX(${offset}px)\` }}` (or Y if axis='y').
//  * - On release, we call `onCommit('next'|'prev')` if swipe is strong/long enough, else `onCancel()`.
//  *
//  * It does NOT manage your slide index. That stays in your component.
//  */
// export function useDragToSlide({
//   axis = 'x',                        // 'x' | 'y'
//   enabled = true,                    // disable/enable the whole behavior
//   thresholdPx,                       // pixel distance to trigger (if omitted, uses fractionOfSize)
//   fractionOfSize = 0.25,             // fallback threshold when thresholdPx is not provided (25% of width/height)
//   velocityThreshold = 0.65,          // px/ms to trigger on quick flick
//   rubberband = true,                 // allow a little resistance beyond edges while dragging
//   lockScroll = true,                 // prevent vertical scrolling once horizontal intent is detected
//   getSize,                           // optional: () => elementSize; if not provided, uses target's clientWidth/Height
...
//     const elRef = useRef(null);             // the element we bind handlers to
...
//     const primary = (e) => (isX ? e.clientX : e.clientY);

//     const computeSize = useCallback((target) => {
//         if (typeof getSize === 'function') return getSize();
//         if (!target) return 1;
//         return isX ? target.clientWidth : target.clientHeight;
//     }, [getSize, isX]);

//     const getThreshold = useCallback((target) => {
...
//         // Opt-in hint for the browser: allow horizontal panning
//         if (lockScroll && target.style && isX) {
//         // You can also set this in CSS: `touch-action: pan-y` on the draggable element.
//         target.style.touchAction = 'pan-y';
//         }
//     }, [enabled, lockScroll, isX]);

//     const onPointerMove = useCallback((e) => {
//         if (!enabled || !downRef.current) return;

//         const target = elRef.current || e.currentTarget;
//         const { x: sx, y: sy, t: st } = startPosRef.current;
...
//             if (lockScroll && isX && Math.abs(dx) > Math.abs(dy) && !preventedScrollRef.current) {
//             // Prevent vertical scroll once horizontal intent is clear
//             e.preventDefault?.();
...
//         // Live offset with optional rubberband
//         const rawDelta = isX ? dx : dy;

//         // Optionally apply a little resistance as the drag grows
...
//         // This keeps small drags 1:1 and adds gentle resistance to very large pulls.
//         }

//         setOffset(nextOffset);
//     }, [enabled, isX, computeSize, lockScroll]);
...
//         const target = elRef.current || e.currentTarget;
//         const { x: sx, y: sy, t: st } = startPosRef.current;
//         const { x: ex, y: ey, t: et } = lastRef.current;

//         downRef.current = false;
//         setDragging(false);

//         const dist = (isX ? ex - sx : ey - sy);
//         const dt = Math.max(1, et - st);
...
//             committed = true;
//             if (dist < 0) onCommit?.('next'); else onCommit?.('prev');
//         } else {
//             onCancel?.();
//         }

//         // Smoothly snap back UI offset (consumer can also animate)
//         setOffset(0);
...
//             target.releasePointerCapture(e.pointerId);
//         }
//     }, [enabled, isX, getThreshold, velocityThreshold, onCommit, onCancel]);

//     // Attach listeners via props; consumers can spread {...bind} on their draggable element
//     const bind = useMemo(() => ({
//         ref: elRef,
//         onPointerDown,
//         onPointerMove,
...
//         // Accessibility: allow keyboard as a bonus
//         onKeyDown: (e) => {
//         if (!enabled) return;
//         if (isX && e.key === 'ArrowRight') swipe('next');
//         if (isX && e.key === 'ArrowLeft') swipe('prev');
//         if (!isX && e.key === 'ArrowDown') swipe('next');
//         if (!isX && e.key === 'ArrowUp') swipe('prev');
//         },
//         tabIndex: 0,
//         role: 'application',
//         'aria-roledescription': 'carousel',
//     }), [enabled, isX, onPointerDown, onPointerMove, onPointerUp, swipe]);
